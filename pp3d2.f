...
c*************************************************************************
	...
	real*8 itimee(10000000),rs(10000000),pi
...
!		 time=1*ran2(seed1)*final_time		 
 ...
 34		    rr=ran2(seed1)
		 ...
		 u1=ran2(seed1)
		 u2=ran2(seed1)
		 q(nevent-i) = 9 + 1*DSQRT(-2.0D0 * LOG(u1)) *
     &        DCOS(2.0D0 * pi * u2)
...
!!      THINNING 
	   ...
	   do jj=2,nevent
	      ...
	      do ii=jj-1,1,-1
!       dtmax=tau2*10**(alpha2*(7.5-q(jj)))
		 dtmax=tau1
		 dt=itime(jj)-itime(ii)
		 if(dt.gt.dtmax)exit

c	      if(q(ii).gt.q(jj))then
c		 nrem(idt,im)=nrem(idt,im)+1
c		 goto 55
c		 qc=q(ii)-(1./alpha2)*log10(dt/tau2)
c!		 qc=q(ii)-1.2*log10(dt)+2.5 ! 0.5
		 qq=q(jj)
		 qc=q(ii)
 		 prob=0.5+0.5*erf((qq-qc)/stdv)
c$$$ 		 if(qq-qc.ge.0)then
c$$$		    prob=1	!
c$$$		 else
c$$$		    prob=1-erf((qc-qq)/stdv)
c$$$		 endif
