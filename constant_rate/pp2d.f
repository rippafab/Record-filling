...
c*************************************************************************
	...
	real*8 aveq,aveqp,aveqpp,p_threshold,total_int,int1,int2
...
c	nmain=1200*10*1.9 !*10*realiz
	...
	tau1=520.00000000001
	alpha2=0.75!/2.5!2.5/2.5*0.85
	tau2=2  !2.2!10**(-alpha*3.5*1.)*3600*24*0.000232!0.001
	stdv=.0001!sqrt(4*0.3)
	pp=1.2!1.08		!omori exponent
	alpha=0.9
	AAA=0.06*2*.5
	cc=4.47100!0.024*3600*24 !0.01
...
!		 time=cc*((ran2(seed1))**(1d0/(1d0-pp)))-cc
 ...
 34		    rr=ran2(seed1)
	...
	beta=2.2
        ...
        ! 1. Calcolo degli integrali delle due branche (assumendo alpha, beta /= 1)
        ! Se alpha o beta sono esattamente 1, serve il logaritmo.
        int1 =(y0**(1.-alpha)-ymin**(1.-alpha))/(1.-alpha)
        
        ! Per continuità B = y0**(beta-alpha) * A. 
        ! L'integrale della seconda parte pesato per la continuità è:
        int2=y0**(beta-alpha)*(ymax**(1.-beta)-y0**(1.-beta))/(1.-beta)
        
        total_int = int1 + int2
	A = 1./total_int
        
        ! Probabilità che y cada nella prima branca [ymin, y0]
        p_threshold = A*int1
	!print*,p_threshold,int1,int2
        ! 2. Estrazione numero casuale uniforme
	
        if (rr < p_threshold) then
            ! Inversione della CDF per la prima branca
            ! Trasformiamo u nel range [0, 1] relativo alla prima parte
            u = rr/p_threshold
            y = (u*(y0**(1.-alpha)-ymin**(1.-alpha))
     &      + ymin**(1.-alpha))**(1./(1.-alpha))
	 else
	    !print*,rr
            ! Inversione della CDF per la seconda branca
            ! Trasformiamo u nel range [0, 1] relativo alla seconda parte
            u = (rr-p_threshold)/(1.-p_threshold)
            y = (u*(ymax**(1.-beta)-y0**(1.-beta))
     &	    + y0**(1.-beta))**(1./(1.- beta))
        end if
...
!!      THINNING 
	   ...
	   do jj=2,nevent
	      qc=7*(0.1+.1*sin(4*6.28*itime(jj)/final_time))
	      
	      qq=q(jj)
	      prob=0.5+0.5*erf((qq-qc)/stdv)
		 ...
		 goto 55
	      ...
	      nevent2=nevent2+1
	      q2(nevent2)=q(jj)
!	      if(q2(nevent2).ge.4.5)print*,itime(jj),q(jj),q2(nevent2)
	   !ind(nevent2)=jj
	      itime2(nevent2)=itime(jj)
	      posiz(nevent2)=jj
!	   idt=int(1d0*dlog(itime2(nevent2))/0.1)+1
!	   write(66,*)itime(jj),q(jj)
!	   ndt2(idt)=ndt2(idt)+1
 55	      continue
	   enddo
!	call flush(66)
c$$$	do i=1,nevent2
c$$$	   write(12,*)itime2(i),q2(i)
c$$$	enddo
