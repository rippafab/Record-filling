program legge_cat
  ...
  open(10,file='cat.dat',status='old')
...
99 nev=i-1
  ...
  do i=1,nev
   ...
   enddo
  do i=1,nev-kk
    xlambda=kk*1d0/(time(i+kk)-time(i))
    write(112,*)0.5*(time(i+kk)+time(i)),xlambda
    !if(xlambda>=1)write(*,*)time(i),xlambda
  enddo
  do ix=-100,1000
     if(n(ix)>0)write(111,*)ix*bin,n(ix)*1d0/(bin)!*nev)
  enddo
